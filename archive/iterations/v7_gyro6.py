...
def play_welcome_message():
    """Plays a welcome message to confirm the script has started."""
    try:
        # Initialize pygame mixer if it's not already
        if not pygame.mixer.get_init():
            pygame.mixer.init()
        print("🔊 Playing welcome message...")
        welcome_text = "System activated. Ready to navigate."
        tts = gTTS(text=welcome_text, lang='en')
        # Use a different name to avoid conflicts
        tts.save("welcome_alert.mp3")
        pygame.mixer.music.load("welcome_alert.mp3")
        pygame.mixer.music.play()
        while pygame.mixer.music.get_busy():
            time.sleep(0.1)
        os.remove("welcome_alert.mp3")
        print("Welcome message finished.")
    except Exception as e:
        print(f"Could not play welcome message: {e}")
...
def handle_elevation_alert(elevation_type, alert_type, level):
    ...
    if alert_type == "short_buzzer":
        threading.Thread(target=lambda: [buzzer.on(), sleep(0.2), buzzer.off(), sleep(0.1)]*3, daemon=True).start()
    elif alert_type == "vibration_voice":
        threading.Thread(target=lambda: [buzzer.on(), sleep(0.1), buzzer.off(), sleep(0.1)]*5, daemon=True).start()
        threading.Thread(target=speak_thread, daemon=True).start()
    elif alert_type == "voice_alert":
        threading.Thread(target=speak_thread, daemon=True).start()
    elif alert_type == "mild_vibration":
        threading.Thread(target=lambda: [buzzer.on(), sleep(0.3), buzzer.off(), sleep(0.2)]*2, daemon=True).start()
...
def main():
    ...
    global gyro_print_counter
    # Play the welcome message at startup
