...
gyro_print_counter = 0  # Counter to control gyroscope printing frequency
...
def print_gyro_values_terminal(gx, gy, gz, distance):
    ...
    # Print every 10 iterations to avoid flooding terminal (adjust as needed)
    if gyro_print_counter % 10 == 0:
        ...
        if gyroscope_available:
            ...
            movement_threshold = 5.0  # degrees/sec
            if abs(gx) > movement_threshold or abs(gy) > movement_threshold or abs(gz) > movement_threshold:
                print(f"    >> MOVEMENT DETECTED: Roll: {gx:.1f}°/s, Pitch: {gy:.1f}°/s, Yaw: {gz:.1f}°/s")
            
            # Check for elevation change indication
            if abs(gy) > elevation_threshold:
                print(f"    >> ELEVATION CHANGE THRESHOLD EXCEEDED: Pitch = {gy:.1f}°/s")
        
        # Distance warning
        if distance <= detection_threshold:
            print(f"    >> PROXIMITY ALERT: Object at {distance:.2f}m (threshold: {detection_threshold}m)")
...
print("Gyroscope values will be printed in terminal every 10 iterations.")
...
try:
    while True:
        ...
        # Print gyroscope values and system status in terminal
        ...
        if gyroscope_available:
            cv2.putText(annotated_frame, f"Gyro X: {gx:.1f} Y: {gy:.1f} Z: {gz:.1f} deg/s", 
        ...
        if distance <= detection_threshold:
            ...
            if (current_time - last_detection_time) > detection_cooldown:
                ...
                if detected_objects:
                    print(f"👁️  Detected objects: {detected_objects}")
                    sound_buzzer(buzzer_duration)
                    sleep(0.5)
                    speak_detection_results(detected_objects)
                else:
                    print("👁️  Object detected but couldn't identify clearly")
                    sound_buzzer(buzzer_duration)
                    sleep(0.5)
