...
# Initialize I2C bus for gyroscope[1][2]
try:
    bus = smbus2.SMBus(1)
...
def read_raw_data(addr):
    """Read raw data from MPU6050[1][2]"""
    try:
...
def get_gyro_data():
    """Get gyroscope data in degrees/sec[1][2]"""
...
def detect_elevation_change(prev_gy, threshold=15):
    """Detect elevation change based on gyroscope Y-axis (pitch) changes[1][2]"""
    if not gyroscope_available:
...
def speak_elevation_change():
    """Provide voice feedback for elevation change[1][2]"""
    def speak_thread():
        try:
...
try:
    while True:
        ...
        # Check for elevation changes[1][2]
        ...
        # Add gyroscope status to display[1][2]
        if gyroscope_available:
            gx, gy, gz = get_gyro_data()
